...
public class OrderDetailTemplateRepository implements OrderDetailRepository {
    ...
    private final RowMapper<OrderDetail> orderDetailRowMapper = new RowMapper<OrderDetail>() {
        ...
        public OrderDetail mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            orderDetail.setDiscountPercent(rs.getDouble("할인율"));
    ...
    public int saveOrderDetail(OrderDetail orderDetail) {
        String query = "insert into 주문세부(주문번호, 제품번호, 단가, 주문수량, 할인율) values(?,?,?,?,?)";
      return  jdbcTemplate.update(query,orderDetail.getOrderId(),orderDetail.getProductId(),orderDetail.getUnitPrice(),orderDetail.getOrderQuantity(),orderDetail.getDiscountPercent());
    }
}
