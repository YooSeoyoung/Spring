...
public interface EmployeeRepository extends JpaRepository<Employee,String> {
    ...
    List<Employee> findByDepartmentIdAndPosition(String departmentNumber, String position);

    @Query("select e from Employee e where e.hireDate >= :hiredate")
    List<Employee> getEmployeesByHiredate(LocalDate hiredate);

    @Query("select e from Employee e where e.hireDate = (select e.hireDate from Employee e order by e.hireDate desc limit 1)")
