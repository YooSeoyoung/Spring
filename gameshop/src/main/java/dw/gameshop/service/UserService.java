...
public class UserService {
    ...
    public UserDTO registerUser(UserDTO userDTO) {
        ...
        if (user.isPresent()) { // 이미 존재하는 ID이면 예외 발생하기
    ...
    public boolean validateUser(String username, String password) {  //확인하는 코드
        User user = userRepository.findById(username)
                .orElseThrow(()->new InvalidRequestException("Invalid Username"));
        return passwordEncoder.matches(password, user.getPassword()); // 입력된 비번과 user에 비번과 일치하는 확인(맞으면 true 틀리면 false)
    }
